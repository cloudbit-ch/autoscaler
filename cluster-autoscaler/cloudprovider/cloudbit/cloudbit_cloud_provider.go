...
var _ cloudprovider.CloudProvider = (*cloudbitCloudProvider)(nil)
...
type cloudbitCloudProvider struct {
	manager         *Manager
	resourceLimiter *cloudprovider.ResourceLimiter
}

func newCloudbitCloudProvider(manager *Manager, rl *cloudprovider.ResourceLimiter) (*cloudbitCloudProvider, error) {
	if err := manager.Refresh(); err != nil {
		return nil, err
	}

	return &cloudbitCloudProvider{
		manager:         manager,
		resourceLimiter: rl,
	}, nil
}
...
func (d *cloudbitCloudProvider) NodeGroups() []cloudprovider.NodeGroup {
...
func (d *cloudbitCloudProvider) HasInstance(node *apiv1.Node) (bool, error) {
	return true, cloudprovider.ErrNotImplemented
}

// Pricing returns pricing model for this cloud provider or error if not
// available. Implementation optional.
func (d *cloudbitCloudProvider) Pricing() (cloudprovider.PricingModel, errors.AutoscalerError) {
...
func (d *cloudbitCloudProvider) GetAvailableMachineTypes() ([]string, error) {
	return []string{}, nil
}
...
func (d *cloudbitCloudProvider) NewNodeGroup(
	machineType string,
...
func (d *cloudbitCloudProvider) GetResourceLimiter() (*cloudprovider.ResourceLimiter, error) {
	return d.resourceLimiter, nil
}

// GPULabel returns the label added to nodes with GPU resource.
func (d *cloudbitCloudProvider) GPULabel() string {
	return GPULabel
}

// GetAvailableGPUTypes return all available GPU types cloud provider supports.
func (d *cloudbitCloudProvider) GetAvailableGPUTypes() map[string]struct{} {
	return nil
}

// GetNodeGpuConfig returns the label, type and resource name for the GPU added to node. If node doesn't have
// any GPUs, it returns nil.
func (d *cloudbitCloudProvider) GetNodeGpuConfig(node *apiv1.Node) *cloudprovider.GpuConfig {
	return gpu.GetNodeGPUFromCloudProvider(d, node)
}

// Cleanup cleans up open resources before the cloud provider is destroyed,
// i.e. go routines etc.
func (d *cloudbitCloudProvider) Cleanup() error {
	return nil
}
...
// BuildCloudbit builds the Cloudbit cloud provider.
func BuildCloudbit(
