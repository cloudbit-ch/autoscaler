...
func NewSnapshotService(client cloudbitgo.Client) SnapshotService {
	return SnapshotService{client: client}
}

func (s SnapshotService) List(ctx context.Context, cursor cloudbitgo.Cursor) (list SnapshotList, err error) {
...
func getSpecificSnapshotPath(snapshotID int) string {
	return cloudbitgo.Join(snapshotsSegment, snapshotID)
}
