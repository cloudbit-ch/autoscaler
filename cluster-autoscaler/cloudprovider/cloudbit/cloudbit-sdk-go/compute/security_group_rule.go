...
func NewSecurityGroupRuleService(client cloudbitgo.Client, securityGroupID int) SecurityGroupRuleService {
	return SecurityGroupRuleService{client: client, securityGroupID: securityGroupID}
}

func (s SecurityGroupRuleService) List(ctx context.Context, cursor cloudbitgo.Cursor) (list SecurityGroupRuleList, err error) {
...
func getSecurityGroupRulesPath(securityGroupID int) string {
	return cloudbitgo.Join(securityGroupsSegment, securityGroupID, securityGroupRulesSegment)
}

func getSpecificSecurityGroupRulePath(securityGroupID, ruleID int) string {
	return cloudbitgo.Join(securityGroupsSegment, securityGroupID, securityGroupRulesSegment, ruleID)
}
